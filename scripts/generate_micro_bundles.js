...
function parseArgs(argv) {
  const out = {
    ...
    taskTitle: 'geb creative quality task',
...
function buildBundle(template, args) {
  ...
  const gene = {
    ...
    category: template.category || 'innovate',
  ...
  const event = {
    ...
    id: `event_${id}_${ts}`,
    schema_version: '1.5.0',
    intent: template.category || 'innovate',
    capsule_id: '',
    genes_used: [],
    outcome: {
      status: 'success',
...
function main() {
  ...
  console.log(`generated ${index.length} geb micro bundles into ${args.outDir}`);
}
